...
class _TypesWidgetState extends State<TypesWidget> {
  ...
  Widget getTypes(BuildContext context) {
    return StreamBuilder(
      ...
      initialData: new List<List<dynamic>>(),
      builder:
  ...
  Widget buildContent(BuildContext context, List<List<dynamic>> types) {
    return ListView.builder(
      itemCount: types.length,
      itemBuilder: (context, ind) {
        Company company = types[ind][1];
        CardType tp = types[ind][0];
        return Padding(
  ...
  void deleteType(CardType tp) async {
    ...
    await cca.deleteAllCreditCardsByTypes(tp.id);
