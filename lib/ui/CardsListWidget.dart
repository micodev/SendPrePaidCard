...
class _CardsListWidgetState extends State<CardsListWidget> {
  ...
  void initState() {
    ...
    cardbloc = CardsByTypeBloc(printWidget.typeId.toString());
    super.initState();
  }
            ...
            child: Slidable(
              ...
              actions: <Widget>[
                IconSlideAction(
                  ...
                  icon: Icons.delete,
                  onTap: () {
                    cardbloc.deleteCardById(card.id);
                    if (cards.length == 1) {
  ...
  Future navigate(CreditCard card) async {
    ...
    if (result != null && result.toString() != "error") {
      ...
      logBloc.addLog(Log(
          userId: (await userBloc.getUser(args[1])).id,
          amount: balance,
          label: "دين",
          date: now.millisecondsSinceEpoch));
      creditcardBloc.deleteCardById(int.parse(args[2]));
      cardbloc.getCreditCards();
