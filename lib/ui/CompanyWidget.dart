...
class _CompanyWidgetState extends State<CompanyWidget> {
  ...
  void deleteCompany(Company cmp) async {
    CardTypeAccess cta = CardTypeAccess();
    CreditCardAccess cca = CreditCardAccess();
    List<CardType> cts = await cta.getCardTypesByCompany(cmp.id);
    cts.forEach((ct) async {
      await cca.deleteAllCreditCardsByTypes(ct.id);
    });
    await cta.deleteAllCardTypesByCompany(cmp.id);
